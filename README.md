...
手動で録画する場合は、ログイン後に「録画」タブ下部へ `lv123456789` のような番組 ID または視聴ページの URL を入力し、「録画開始」を押します。手動録画には録画対象への登録やフォローは不要です。終了済み番組はタイムシフトとして先頭から取得します。視聴できない場合は、同じアカウントで公式サイトの視聴可否・予約状況・公開期限を確認してください。

タイムシフトの録画カードには映像・音声の取得率を表示します。100%になってもコメント取得が残る場合があり、両方が終わるまで待ちます。コメントは提供された履歴を取得し、投稿時刻順のCSVに保存します。
