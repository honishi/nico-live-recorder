...
blank の除外、元の番号による AES-128 の暗黙 IV、明示 IV、鍵・初期化情報の切り替えを扱います。取得失敗時は最大4回試行しますが、403・404は再試行しません。セグメントの404は欠落として数え、初期化情報や鍵の404は失敗として停止します。途中失敗・中断では先読み中の要求を中止します。アプリの本体コードは変更していません。
...
一般会員の予約済みケースはまだ未確認です。全編検証の結果は以下に追記します。会員種別の一般的な視聴条件を、この1番組の結果だけで確定しません。
