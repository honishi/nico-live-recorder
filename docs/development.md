...
終了済み放送の検証スクリプトについては [タイムシフト検証手順](timeshift-probe.md)、検証の結論と本体実装へ引き継ぐ範囲は [検証結果と初期対応範囲](timeshift-feasibility.md) を参照してください。 本体への変更方針は [手動タイムシフト録画の組み込み設計](timeshift-recording-design.md) にまとめています（未実装）。
