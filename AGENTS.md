...
- issue の起票は必須ではない。その場で調査・実装・検証まで完結する改修は、機能追加や不具合修正も含め、新しい issue を作らずに進める。文書・規約の修正や一時的な調査も同様。
- GitHub の issue は、後日対応する課題を残す、複数の作業にまたがる進捗を追うなど、継続的な追跡・管理が必要な場合に作る。機能追加や不具合修正という種類だけを理由に自動で起票しない。起票についてユーザーの指定があればそれに従う。
