...
import type { HlsSegment, TrackResult } from '../../src/main/core/nico/hls';
import type { StreamCookie } from '../../src/main/core/nico/watch-protocol';
import type { downloadMetrics as coreMetrics } from '../../src/main/core/nico/timeshift-download';

type TrackDownloader = (
  segments: HlsSegment[],
  sink: Writable,
  cookies: StreamCookie[],
  threads: number,
  signal: AbortSignal,
  metrics: ReturnType<typeof coreMetrics>,
) => Promise<TrackResult>;
...
export function trackSegments(count: number) {
...
// 本番と検証スクリプトの別実装へ、同じ取得・保存・中断の契約を適用する。
export function testTrackDownload({
  downloadTrack,
  downloadMetrics,
  ErrorType,
}: {
  downloadTrack: TrackDownloader;
  downloadMetrics: typeof coreMetrics;
  ErrorType: new (code: string) => Error;
}): void {
  test.each([1, 5])(
  ...
  test.each(['failure', 'abort'])(
    ...
    async (scenario) => {
      ...
      const metrics = downloadMetrics();
      const task = downloadTrack(trackSegments(5), trackOutput().sink, [], 3, stop.signal, metrics);
  ...
  test('出力待機中の中断でも終了する', async () => {
    ...
    const stop = new AbortController();
    const task = downloadTrack(trackSegments(5), sink, [], 1, stop.signal, downloadMetrics());
    const rejected = expect(task).rejects.toThrow();
  ...
  test('並列取得でも鍵・初期化情報の切り替えと明示IVを維持する', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn((url: string) => {
        ...
        if (name.startsWith('key'))
          return Promise.resolve(new Response(keys[Number(name.slice(3))]));
        return Promise.resolve(new Response(encrypted[Number(name)]));
      }),
    );
    const tracks = trackSegments(3).map((segment, index) => ({
      ...segment,
    ...
    const { sink, chunks } = trackOutput();
    await downloadTrack(tracks, sink, [], 5, new AbortController().signal, downloadMetrics());
