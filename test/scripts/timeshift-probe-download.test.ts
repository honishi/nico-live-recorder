...
test('欠落は件数に残し、後続の番号を詰めない', async () => {
  ...
  const metrics = downloadMetrics();
  const result = await downloadProbeTrack(
    segments(3),
    output().sink,
    [],
    5,
...
test.each(['failure', 'abort'])(
  ...
  async (scenario) => {
    ...
    const metrics = downloadMetrics();
    const task = downloadProbeTrack(segments(5), output().sink, [], 3, stop.signal, metrics);
...
test('出力待機中の中断でも終了する', async () => {
  ...
  const stop = new AbortController();
  const task = downloadProbeTrack(segments(5), sink, [], 1, stop.signal, downloadMetrics());
  const rejected = expect(task).rejects.toThrow();
...
test('並列取得でも鍵・初期化情報の切り替えと明示IVを維持する', async () => {
  ...
  const { sink, chunks } = output();
  await downloadProbeTrack(tracks, sink, [], 5, new AbortController().signal, downloadMetrics());
